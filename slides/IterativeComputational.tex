...
\begin{exerciseframe}
  ...
  \end{itemize}
    ...
    Analyze communication and compuputation complexity
    and weak scaling efficiency. Show that this does not scale weakly.
    
  Case 2. Divide the domain into patches of size
  %
  $(n/\sqrt p)\times (n/\sqrt p)$.
  %
  Show that this case does scale weakly.

  For the proposed machine above, derive concrete numbers
  for a domain of one-million-by-one-million points over one thousand processors.
\end{exerciseframe}
