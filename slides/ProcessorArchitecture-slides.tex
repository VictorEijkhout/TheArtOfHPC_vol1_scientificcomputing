...
\begin{numberedframe}{Vector instructions}
  \begin{itemize}
  \item Multiple (independent) operands per cycle
  \item Similar to pipelining but not the same
  \item `Vector width' is 2,4,8.
  \end{itemize}
\includegraphics[scale=.3]{simd-add}
\end{numberedframe}

\begin{numberedframe}{DAXPY}
  BLAS operation `A-x plus y':
\begin{lstlisting}
for ( i )
    y[i] = a[i]*x + y[i]
\end{lstlisting}
\begin{itemize}
\item \lstinline{x} stays in register
\item two loads and one store per operation
\item candidate for pipelining or vector instructions
\end{itemize}
\end{numberedframe}
\begin{numberedframe}{DAXPY performance}
\small
...
\begin{numberedframe}{The Big Story}
  ...
  \item This helps if data is reused: second access is faster
...
\begin{numberedframe}{Memory hierarchy}
  \includegraphics[scale=.4]{spr_specs}
\end{numberedframe}
...
\begin{numberedframe}{Associativity}
  \includegraphics[scale=.4]{associativity-icx}

  Associative cache structure of Intel Ice Lake.
\end{numberedframe}
