...
The C standard library has a file \indexheader{assert.h} which provides an
\lstinline{assert()} macro.
Inserting \lstinline{assert(foo)} has the following effect:
if \lstinline{foo} is zero (false), a diagnostic message is printed
on standard error:
...
\end{lstlisting}
The \lstinline{assert} macro can be disabled by defining the
\lstinline{NDEBUG} macro.
...
The \ac{CPP} has built-in macros
\indextermtt{__LINE__}~/ \indextermtt{__FILE__}
...
C++ has the containers such as \lstinline{std::vector} which support bound checking:
\begin{lstlisting}
vector<float> x(25);
x.at(26) = y; // throws an exception
\end{lstlisting}
On the other hand, the C-style \lstinline{x[26]} does not perform such checks.

\Level 2 {Fortran}

Fortran arrays are more restricted than C arrays, so compilers
often support a flag for activating runtime bounds checking.
For \indexterm{gfortran} that is \lstinline{-fbounds-check}.

\Level 1 {Memory leaks}
%%packtsnippet memoryleak
...
\begin{lstlisting}
for (i=.... ) {
  float *block = malloc( /* large number of bytes */ )
...
C++~standard containers have largely made this problem go away.
%%packtsnippet end
...
function \indextermtt{mcheck},
defined in \indexheader{mcheck.h}, that has a
similar function.)

If you write in~C, you will probably know the \lstinline{malloc} and \lstinline{free} calls:
\begin{lstlisting}
int *ip;
...
\end{lstlisting}
Now you can write your own \lstinline{free}, which tests whether the bounds of
...
list. You can then write a macro \lstinline{CHECKMEMORY} which tests all your
...
(Instead of writing a wrapper for \lstinline{malloc}, on some systems you can
influence the behavior of the system routine. On linux, \lstinline{malloc}
