...
\lstset{language=shell,style=\snippetstyle}
...
\item Within Linux there are various
  \index{distribution|see{Linux, distribution}}\indextermbus{Linux}{distribution}s
...
\begin{exercise}
  ...
  \lstinline{ls name} on one of those.
  By using an option, for instance \lstinline{ls -s name}
  you can get more information about \lstinline{name}.
\end{exercise}
...
In addition to the above \lstinline{ls -s} for the size, there is
\lstinline{ls -l} for the `long' listing.
...
%\practical
\begin{exercise}
  Type \lstinline{cat > newfilename} (where you can pick any filename) and
  ...
  contents of that file: \lstinline{cat newfilename}.
\end{exercise}
\begin{outcome}
...
\begin{caution}
  Be sure to type \n{Control-d} as the first thing on the last line of input. If you
    ...
    start creating a file with \lstinline{cat > filename} and hit \n{Control-c} in
...
\begin{exercise}
  Read the man page of the \lstinline{ls} command:
  \lstinline{man ls}.
  Find out the size and the time~/ date of the last change
  to some files, for instance the file you just created.
\end{exercise}
\begin{outcome}
  Did you find the \lstinline{ls -s} and \lstinline{ls -l} options? The first one
...
a file if it already exists. Use \lstinline{ls -l} to confirm this behavior.

\Level 2 {\texttt{cp, mv, rm, ln}}

The \indexunix{cp} can be used for copying a file (or directories,
see below): \lstinline{cp file1 file2} makes a copy of \lstinline{file1} and names it
\lstinline{file2}.

%\practical
\begin{exercise}
  Use \lstinline{cp file1 file2} to copy a file. Confirm that the two files
...
\end{outcome}
\begin{caution}
  If \lstinline{file2} already exists, you
...
Files are deleted with \lstinline{rm}. This command is dangerous: there is no
undo.
For this reason you can do \lstinline{rm -i} (for `interactive') which
asks your confirmation for every file.
See section~\ref{sec:shell-wildcard} for more aggressive removing.

Sometimes you want to refer to a file from two locations.
This is not the same as having a copy: you want to be able
to edit either one, and have the other one change too.
This can be done with \indexunix{ln}: `link'.
...
%\practical
\begin{exercise}
  Do \lstinline{ls /usr/share/words} or \lstinline{ls /usr/share/dict/words} to
  confirm that a file with words exists on your system. Now experiment
  with the commands \lstinline{head}, \lstinline{tail}, \lstinline{more}, and \lstinline{wc} using
  that file.
\end{exercise}
\begin{outcome}
...
\begin{remark}
  The \lstinline{tail} command can display the end of a file `as it grows':
  do \lstinline{tail -F some_log_file}.
\end{remark}
...
%\practical
\begin{exercise}
  Do \lstinline{file foo} for various `foo': a text file, a directory, or the
  \lstinline{/bin/ls} command.
\end{exercise}
\begin{outcome}
...
\lstinline{ls /} to see what the files and directories there are in the
...
\end{exercise}
\begin{outcome}
  You will typically see something like \lstinline{/home/yourname} or
  \lstinline{/Users/yourname}. This is system dependent.
\end{outcome}

Do \lstinline{ls} to see the contents of the working directory. In the
displays in this section, directory names will be followed by a
slash:~\lstinline{dir/} but this character is not part of their name.  You can
get this output by using \lstinline{ls -F}, and you can tell your shell to
use this output consistently by stating \lstinline{alias ls=ls -F} at the start of your
session. Example:
...
%\practical
\begin{exercise}
  Make a new directory with \indexunix{mkdir}~\lstinline{newdir} and view
  the current directory with \lstinline{ls}.
\end{exercise}
\begin{outcome}
...
## et cetera
\end{verbatim}
but it's shorter to use the \n{-p} option (for `parent') and write:
\begin{verbatim}
mkdir -p sub1/sub2/sub3
\end{verbatim}
which creates any needed intermediate levels.
The \n{-p} option can also be used if the directory already exists.
(What happens normally if you try to \lstinline{mkdir} a directory
that already exists?)
...
the following ways:
\begin{itemize}
\item \lstinline{cd} Without any arguments, \lstinline{cd} takes you to your home directory.
\item \lstinline{cd <absolute path>} An absolute path starts at the root of
  the directory tree, that is, starts with~\lstinline{/}. The \lstinline{cd} command
  takes you to that location.
\item \lstinline{cd <relative path>} A relative path is one that does not
  start at the root. This form of the \lstinline{cd} command takes you to
  \lstinline{<yourcurrentdir>/<relative path>}.
\end{itemize}

%\practical
\begin{exercise}
  Do \lstinline{cd newdir} and find out where you are in the directory tree
  with \lstinline{pwd}.  Confirm with \lstinline{ls} that the directory is empty.  How
  would you get to this location using an absolute path?
\end{exercise}
\begin{outcome}
  \lstinline{pwd} should tell you \lstinline{/home/you/newdir}, and \lstinline{ls} then has no
  output, meaning there is nothing to list.  The absolute path is
  \lstinline{/home/you/newdir}.
\end{outcome}

%\practical
\begin{exercise}
  Let's quickly create a file in this directory: \indexunix{touch}
  \lstinline{onefile}, and another directory: \lstinline{mkdir otherdir}.
  Do \lstinline{ls} and confirm that there are a new file and directory.
\end{exercise}
\begin{outcome}
...
The \lstinline{ls} command has a very useful option: with \lstinline{ls -a} you see
your regular files and hidden files, which have a name that starts
with a dot. Doing \lstinline{ls -a} in your new directory should tell you that
...
%\practical
\begin{exercise}
  Predict where you will be after \lstinline{cd ./otherdir/..}  and check to
  see if you were right.
\end{exercise}
\begin{outcome}
  The single dot sends you to the current directory, so that does not
  change anything. The \lstinline{otherdir} part makes that subdirectory your
  current working directory. Finally, \lstinline{..} goes one level back. In
...
\lstinline{cd}'ing to it: \lstinline{cd} without arguments, \lstinline{cd ~}, and \lstinline{cd \$HOME}
  all get you back to your home.

Go to your home directory, and from there do \lstinline{ls newdir} to check
the contents of the first directory you created, without having to go
there.

%\practical
\begin{exercise}
  What does \lstinline{ls ..} do?
\end{exercise}
\begin{outcome}
  Recall that \lstinline{..} denotes the directory one level up in the tree:
...
\item Go to your home directory:~\lstinline{cd}\\
  How do you move \lstinline{sub1/a} to here?
\end{enumerate}
\end{exercise}
...
\n{cp -r newdir somedir}
where \n{somedir} is an exiting directory,
and
...
\begin{zshnote}
  ...
  is an error if there are no such files.
...
\Level 1 {Line manipulation with \protect\texttt{cut} and \protect\texttt{tr}}

Two useful utilities for `editing' a line are \lstinline{tr} and \lstinline{cut}.

First \indexunix{tr} which stands for `translate'.
It takes two characters or ranges of characters:
the first character, or any character in the first range,
is translated to the second, or the corresponding character in the second range.
...
\begin{exercise}
  Let \lstinline{a} and \lstinline{b} be strings.
  How do you test if they are equal, up to differences in case?
  Can you do this as a single command?
  If you know how to write shell scripts,
  write one that accepts two arguments and return true if they
  are equal up to case differences.
\end{exercise}

You can also use \indexunix{tr} to delete, rather than translate,
characters:
\begin{lstlisting}
...
\begin{exercise}
  Referring to section~\ref{sec:error-redirect},
  how can you prevent the error message from \lstinline{touch}
  to show on your terminal?
\end{exercise}
...
\begin{stufflearned}[Standard file]
  \texttt{stdin} (number \texttt{0})& is the file that provides input for processes.\\
  \texttt{stdout} (number \texttt{1})& is the file where the output of a process is  written.\\
  \texttt{stderr} (number \texttt{2})& is the file where error output is written.\\
\end{stufflearned}
...
Hint: see section~\ref{tut:unix-backquote}.
\end{exercise}
...
\Level 1 {Error redirection}
\label{sec:error-redirect}

The \lstinline{stdout} and \lstinline{stderr} files are treated very similar
by default: they both go to the terminal output.
And normally that is fine, but suppose you run a program that generates errors
and pipe it to \lstinline{less}:
\begin{lstlisting}
./myprogram | less
\end{lstlisting}
If you scroll back you'll see that you can not revisit the error messages.
Both  \lstinline{stdout} and \lstinline{stderr} went to your screen,
but only  \lstinline{stdout} went to \lstinline{less}.

To manipulate \lstinline{stderr} you need to use the stream number
`\lstinline{1}'~for \lstinline{stdout}, and 
`\lstinline{2}'~for \lstinline{stderr}.
With this, you can redirect the two streams separately.
For instance,
\begin{lstlisting}
...
If you want to ignore errors, send them to \indexunix{/dev/null},
which is writable file that is not actually stored.
So:
\begin{lstlisting}
./myprogram 2>/dev/null
\end{lstlisting}

In the first example of this section having separate treatment
was not the solution, but rather the problem.
To send both streams to a pipe, use
\begin{lstlisting}
./myprogram 2>&1 | less
\end{lstlisting}
If you want to send them both to the same file,
you need the somewhat counterintuitive sequencing:
\begin{lstlisting}
...
\begin{stufflearned}[Idiom]
  ...
  \n{program 2>&1 | less}& pipe output and errors to \n{less}\\
\end{stufflearned}

\Level 1 {\texttt{tee}}

What if you want to send output to a file, but also view it on your screen
as it's being generated?
For that, use \indexunixdef{tee}.
(The name comes from `T-joins where one tube --~hooked up to the vertical leg
of the~`T'~-- can be connected to two others, hooked up to the left and right horizontal
bar.)
So:
\begin{lstlisting}
./myprogram | tee my.out
\end{lstlisting}
sends output to \lstinline{my.out} and displays it on screen.

\begin{exercise}
  \begin{itemize}
  \item How would you send output to a file and to \lstinline{less}?
  \item How would you send  \lstinline{stdout} and \lstinline{stderr}
    both to~\lstinline{tee}?
  \end{itemize}
\end{exercise}
...
\item The brackets of the conditional need to have spaces surrounding
  them.
\item There is no semicolon after \indexunix{then} or \indexunix{else}:
...
Modern versions of Bash have an extended conditional syntax
...
Here is a short summary of possible tests,
given through examples:
\begin{lstlisting}
if [ $x = "foo" ]               ## text equality
if [ $x -eq 3 ]                 ## numerical equality; also `-gt' &c
if [ ! -z "${x}" ]              ## test if variable is not null
if [ ! -z "${x}" -a -z "${y}" ] ## `and' of two tests, also `-o'
if [ -d foo ]                   ## is there a `foo' directory
if [[ -x foo ]]                 ## is `foo' executable
if [[ $myvar == *substring* ]]  ## pattern matching
\end{lstlisting}
...
see section~\ref{tut:unix-backquote}.

\begin{exercise}
  Take a program that reads one number from standard in, and run it in a loop in the shell,
  using the number \texttt{10} through~\texttt{20} as input.
\end{exercise}

You can break out of a loop with \indexunix{break};
...
You can think of a script as putting single statements
together in a file so that you can repeatedly execute them.
The simplest Unix mechanism to do this bundling goes like this:
\begin{enumerate}
\item Make a file \lstinline{my_commands.txt} with any Unix commands;
\item Then
\begin{lstlisting}
...
\end{lstlisting}
\end{enumerate}
The effect will be as if you had typed the commands
straight at the command prompt.

While this can be useful, it can also be confusing
since the commands file can do \lstinline{cd}
or make other changes to your environment.
Usually it's better to let a script run
in its own sheltered world~-- to be precise: in a subshell
...
\item Using
  ...
  \verb+myscript "1 2 3"+.
\item Using
  \verb+"$@"+ preserves quotes:
  \verb+myscript "1 2" 3+.
\end{itemize}

If you want to write your own commandline parsing,
so that you could write
\begin{lstlisting}
./myscript.sh -a -b arg
\end{lstlisting}
another useful command is \indexunix{shift}.
If you call this, the internal list of arguments is modified:
\begin{enumerate}
\item the first argument is removed, and
\item the argument count is lowered.
\end{enumerate}
Supposing you have a script with some (optional) flags
...
\end{lstlisting}
should print the message in uppercase.
Make sure that the order of
the arguments does not matter, and give an error message for any
unrecognized option.
\end{exercise}

\Level 1 {`Here' documents}

Sometimes it's necessary to create a file (often a script)
from a running script. To this purpose,
there are \indextermp{here document}.
This is a mechanism of sending multiple lines as input
to a command.
The lines are delimited by
two occurrences of an arbitrary string,
its first occurrence marked by \lstinline+<<+:
\begin{lstlisting}
somecommand <<ASTRING
lines
lines
ASTRING
\end{lstlisting}
Typically, the command is \lstinline{cat},
and you use this to generate a new document
containing the value of some dynamically defined variables.
\begin{lstlisting}
#!/bin/bash
# this file: makesay.sh
...
chmod +x say.sh
\end{lstlisting}
Now running \lstinline{makesay.sh foo}
generates a file \lstinline{say.sh} that contains \lstinline{echo foo}.

\begin{remark}
  You can prevent expansion of variables by
...
\begin{remark}
  In addition to `here documents', there are also
  \indextermp{here string}:
\begin{lstlisting}
some_command <<<"some string"    
\end{lstlisting}
is equivalent to
\begin{lstlisting}
echo "some string" | some_command
\end{lstlisting}
\end{remark}

\Level 1 {Error handling}

Scripts, like any other type of program,
can fail with some runtime condition.

\begin{enumerate}
\item
  If there is no clear error message,
  you can at least rerun your script with a line
\begin{lstlisting}
set -x
\end{lstlisting}
which echoes each command to the terminal before execution
\item Each script has a return code,
  contained in the variable~\lstinline+$?+
  which you can inspect.
\item It is also possible that some command in the script
fails, but the script continues running.
You can prevent this with 
\begin{lstlisting}
set -e
\end{lstlisting}
which makes the script abort if any command fails.
...
Here is an idiom for being a little more informative
about errors:
\begin{lstlisting}
errcode=0
...
\end{lstlisting}
The crucial second line contains an `or' condition:
either \lstinline{some_command} succeeds,
or you set \lstinline{errcode} to its exit code.
This conjunction always succeeds, so now you can inspect
the exit code.
...
\Level 1 {Brace expansion}

If you need to operate on a number of files with similar names,
but which can not be describe with a wildcard, you can use brace expansion.
In between braces you give the explicit list of variants in the names:
...
\Level 1 {Tilde expansion}

The tilde gives your own, or someone else's home directory:
\begin{lstlisting}
$ echo ~
...
\Level 1 {Variable expansion}

This gives the value of shell variables:
\begin{lstlisting}
$ x=5
$ echo $x
5
...
\Level 1 {Command manipulation}

\Level 2 {History expansion}

The shell keeps a history of your previously executed commands.
The command \indexunix{history} gives you the list,
each command prefixed with its number in the command history.
Let's say this is part of your history:
\begin{lstlisting}
 1669  make format
 1670  make clean
 1671  ls foo*
 1672  hostname
\end{lstlisting}
You can now re-execute previous commands,
either exactly or with modifications,
through a number of mechanisms.
\begin{itemize}
\item You can refer to a command by its number: \lstinline+!1671+
  will execute \lstinline+ls foo*+ again.
\item You can refer to a command by its initial characters:
  \lstinline+!make+ will execute the last make command,
  in this case \lstinline+make clean+.
\item Should you want to re-execute a command, but its initial
  characters are not unique, then you can refer to a unique non-initial substring:
  \lstinline+!?format?+ will execute the last command containing \lstinline{format}.
\end{itemize}
If you want to modify a previous commandline, retrieved by any of the above,
there are several possibilities:
\begin{itemize}
\item You can display a previous command without executing it:
  \lstinline+!make:p+ will echo the command but not execute it.
  You can then use arrow-up to make it available for editing.
\item If you mistype a command, say \lstinline+make fromat+, you can easily edit
  it with \lstinline+^fro^for^+ to replace a substring by another string.
\item Another way of editing a command is by a \indexterm{sed}-like syntax:
  \lstinline+!!:s/fro/for/+
\item These mechanisms can be combined: if you are not sure whether an edit
  gave the right result, display the result as \lstinline+!!:s/fro/for/:p+
\item Finally, a global replace on a previous command is possible:
  \lstinline+!make:s/a/b/:g&+
\end{itemize}
And there are more options, but these are a good basic set.

\Level 2 {Command substitution}
...
\Level 1 {System information}

Most of the above material holds for any Unix or Linux system.
Sometimes you need to know detailed information about the system.
The following tell you something about what's going on:
\begin{itemize}
...
Sometimes you want to know what system you are actually using.
Usually you can get some information out of \indexunix{uname}:
\begin{lstlisting}
$ uname -a
Linux staff.frontera.tacc.utexas.edu 3.10.0-1160.45.1.el7.x86_64 #1 SMP Wed Oct 13 17:20:51 UTC 2021 x86_64 x86_64 x86_64 GNU/Linux
\end{lstlisting}
This still doesn't tell you what \indextermbus{Linux}{distribution} you are on.
For that, some of the following may work:
\begin{lstlisting}
$ lsb_release -a
...
changing the owning user of a file with \indexunix{chown}
...
Change the owning user of a file is done with \indexunixdef{chown}:
\begin{lstlisting}
