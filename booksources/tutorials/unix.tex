...
%\practical
\begin{exercise}
  If the \indextermunix{ls} command shows that there are files, do
...
The \indextermunix{cat} command (short for `concatenate')
...
\begin{exercise}
  ...
  press the \texttt{d} key.  Now use \indextermunix{cat} to view the
  contents of that file: \n{cat newfilename}.
\end{exercise}
\begin{outcome}
  In the first use of \indextermunix{cat}, text was appended
...
is the \indextermunixdef{man} command, for `manual'.
...
\end{exercise}
\begin{outcome}
  \indextermunix{head} displays the first couple of lines of a file,
  \indextermunix{tail} the last, and \indextermunix{more} uses the
  ...
  output.  The \indextermunix{wc} (`word count') command reports the
...
\begin{exercise}
  ...
  \indextermunix{pwd}. Your home directory is your working directory
...
%\practical
\begin{exercise}
  Make a new directory with \indextermunix{mkdir}~\n{newdir} and view
...
%\practical
\begin{exercise}
  Let's quickly create a file in this directory: \indextermunix{touch}
...
The \indextermunix{tar} command stands for `tape archive',
that is, it was originally meant to package files on a tape.
(The `archive' part derives from the \indextermunix{ar} command.)
...
adding the \n{z} compressiong for \indextermunix{gzip}
is a good idea:
\begin{verbatim}
...
\begin{exercise}
  ...
  files in a system directory. Do \indextermunix{which}~\n{ls}, and do
  an \n{ls -l} on the result.
\end{exercise}
\begin{outcome}
...
\item Variables that are defined by control structures such as \indextermunix{for};
  see below.
\end{itemize}
...
\begin{remark}
  This does not include variables you define yourself, unless you \indextermunix{export} them;
  see below.
\end{remark}
...
there is the \indextermunix{unset} command.
...
\begin{exercise}
  Bash conditionals have an \indextermunix{elif} keyword.
...
contained in \indextermunix{PATH}; see section~\ref{sec:PATH}.
...
basis.
The stream editor \indextermunix{sed} is one example. If you have used the
