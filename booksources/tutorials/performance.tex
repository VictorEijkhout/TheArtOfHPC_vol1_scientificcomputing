...
\lstinputlisting[language=Fortran]{code/papi/timers/ftimer.F}
with output
...
\Level 1 {Parallel timers in MPI and OpenMP}
...
% ./MyProgram
\end{lstlisting}
leaving behind a file \indextermtt{gmon.out}.
...
Profile generation is done by prefix your commandline with \lstinline{perf record}.
Various options exist, for instance for methods of call graph generation:
\begin{lstlisting}[language=shell]
perf record myprogram myoptions
perf record --call-graph fp myprogram myoptions
\end{lstlisting}
By default, event information is gathered into a file \n{perf.data},
...
\end{lstlisting}
The output directory can be specified with the \lstinline{-r} option.
The default name uses a coding \lstinline{r999xx}
where \lstinline{999} numbers from \lstinline{000},
and the \lstinline{xx} encodes the collection type,
such as \lstinline{hs} for `hotspots'.

Some useful options:
\begin{itemize}
\item \lstinline{-report-knob show-issues=false}: skip reporting of
  low CPU utilization and other non-issues.
\item \lstinline{-format csv -csv-delimiter comma}:
  generate comma-separated output for easy post-processing.
\item \lstinline{-column "Function,CPU Time"}: selective reporting.
\end{itemize}

%% https://www.intel.com/content/www/us/en/docs/vtune-profiler/user-guide/2023-0/generating-command-line-reports.html

For graphical output you can use \indextermtt{vtune-gui}.
