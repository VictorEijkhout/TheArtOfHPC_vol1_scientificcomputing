...
In this tutorial you will learn \indextermunixdef{git},
...
and \emph{Microsoft Sharepoint}\index{Sharepoint}\index{Microsoft!Sharepoint|see{Sharepoint}}.
...
and \nameunix{git} is by far the most popular system these days.
...
which gets synchronized to a \indextermsub{remote}{repository}.
...
\indextermtt{github.com} or \indextermtt{gitlab.com},
...
As you see, even an empty repository contains a directory \indextermunix{.git}.
...
(we haven't discussed branches yet; see below)
was called `master'.
...
the \nameunix{git} software does not do this, as of this writing in~2022.
...
\begin{gitstep}{firstfile.runout1}
  ...
  Here we use \lstinline{echo} as a short cut.
  The file will initially be untracked.
\end{gitstep}

You need to \lstinline{git add} on your file to tell \nameunix{git} that the file
...
It is easiest to supply a short description with the \lstinline{-m} option;
without this option \nameunix{git} will open an editor
(to be precise: the value of the variable \indextermunix{EDITOR})
for you to type one.
...
%% the \lstinline{git status} command will list it as `modified'.
%% You then need to \lstinline{git add} and \lstinline{git commit} it
%% to get the changes into the repository.
...
\begin{gitstep}{changefirst.runout2}
  See what the changes are compared to the previously committed version.
\end{gitstep}
...
If you have edited multiple files you can specify those with a wildcard.
You can also specify to add all updated files:
\begin{lstlisting}
git add -u :. ## updated, already added files in the current directory
git add -u :. ## updated, already added files in the whole repository
\end{lstlisting}
...
\begin{lstlisting}
git remote -v
# output: origin  https://username@github.org/username/reponame.git
git remote set-url origin git@github.org:username/reponame.git
\end{lstlisting}
...
There can be various reasons for \nameunix{git} to report a conflict.
\begin{itemize}
...
As before, we have directories \lstinline{person1} and \lstinline{person2}
containing independent clones of a repository.
We have a file of four lines long, but contrary to above,
we now make edits that are too close together for \nameunix{git}'s
...
\begin{gitstep}{conflict.runout2}
  ...
  we use an edit by \indextermunix{sed}, changing \lstinline{1} on line~1 to \lstinline{one}.
...
\begin{gitstep}{conflict.runout5}
  ...
  Edit this file, commit the merged file and push.
\end{gitstep}
...
\item you make a pull request.
\end{enumerate}

It is possible to do this whole workflow from the commandline,
but sites such as \indexterm{github} and \indexterm{gitlab}
make this far easier.
...
\nameunix{git} will not be able to merge
and you have to edit the file as in section~\ref{sec:git-conflict}.

\begin{figure}[ht]
  \includegraphics[scale=.4]{gitflow}
  \caption{Git workflow of adding (top arrows) and pulling (bottom)}
  \label{fig:git-flow}
\end{figure}
...
You can do this by 
\begin{enumerate}
\item Attaching \indexgit{tag} to the state of the repository, or
\item Creating an \indexgit{archive}: a released version that
...
\begin{remark}
  Hosting sites such as \indexterm{GitHub} make
  tagged releases easy through a web interface.
\end{remark}

\Level 1 {Archives, releases}

%% https://git-scm.com/book/en/v2/Distributed-Git-Maintaining-a-Project#_preparing_release

If you want to make a released version of your software,
that can be downloaded and does not rely on the \nameunix{git} software,
use the \indexgit{archive} command:
\begin{lstlisting}
...
%% hg push test
pushing to https://VictorEijkhout:***@github.org/
...
\practical{Do a \lstinline{cat} on the file that both of you have been editing.
...
For conflicts in binary files, \nameunix{git} has a flag where you can indicate
