...
latency and computation stays the same.

Since every processor originates a
minimum spanning tree, we have $\log_2p\alpha$ latency.
For the bandwidth we reason as
...
Again we assume that gathers with multiple targets are active simultaneously.
Since every processor originates a
minimum spanning tree, we have $\log_2p\alpha$ latency;
since each processor receives $n/p$ elements from $p-1$ processors,
there is $(p-1)\times(n/p)\beta$ bandwidth cost.
The total cost for constructing a
...
of its \indexterm{halo region}. Under reasonable assumptions on the partitioning
