...
This means that on cache-contained datasets performance is limited by the bandwidth
from that cache,
and for streaming operations such as operations on long vectors
by the bandwidth from main memory.
If we would plot performance as a function of dataset size, you would expect
a step graph, with larger datasets at a lower bandwidth and performance level.

Let's make an attempt to show such behavior.
...
\item Modern processors have multiple cores,
  and bandwidth to main memory is shared between them.
  Running the streaming kernel on only one core is likely to give
  a higher per-core bandwidth
...
\begin{figure}[t]
  ...
  \begin{multicols}{2}
    \tikzsetnextfilename{multicore-bandwidth-tot-icx}
    \begin{tikzpicture}
      \begin{axis}[
          title=Skylake,
          ylabel=bandwidth, xlabel=cores, 
          width=.45\textwidth, height=.45\textwidth,
          legend entries={8K,80K,800K,8M,},
          legend pos=north west,
        ]
        \addplot table[y=8K]{plots/bwtotal-stampede3-icx.csv};
        \addplot table[y=80K]{plots/bwtotal-stampede3-icx.csv};
        \addplot table[y=800K]{plots/bwtotal-stampede3-icx.csv};
        \addplot table[y=8M]{plots/bwtotal-stampede3-icx.csv};
      \end{axis}
    \end{tikzpicture}
   \columnbreak
   \tikzsetnextfilename{multicore-bandwidth-thr-icx}
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot table[y=8K]{plots/bwthread-stampede3-icx.csv};
        \addplot table[y=80K]{plots/bwthread-stampede3-icx.csv};
        \addplot table[y=800K]{plots/bwthread-stampede3-icx.csv};
        \addplot table[y=8M]{plots/bwthread-stampede3-icx.csv};
      \end{axis}
    \end{tikzpicture}
  \end{multicols}
  %% \addplot table[y=linear]{plots/fivepoint-scaling-frontera6000.csv};
  \caption{Intel Icelake bandwidth (Gbyte/sec) as a function of core counts and data set sizes, left: Aggregate, right: Per-core}
  \label{fig:aggregate-bandwidth}
\end{figure}

To get a realistic evaluation, we instead look
at the aggregate bandwidth over all available cores.
%
  \Level 1 {Aggregate bandwidth}
%
We explore the above factors by running the streaming kernel
on multiple stream lengths and multiple core counts.
%
\cxxverbatimsnippet{bandwidththreads}

We test this on the \indextermbus{TACC}{Stampede3} cluster,
with dual-socket \indextermbus{Intel}{Ice Lake} processors,
with a total of 112 cores per node.
%
Figure~\ref{fig:aggregate-bandwidth} shows that
for any dataset size the aggregate bandwidth grows with the core count,
but the individual per-core bandwidth goes down.
With the largest dataset size, aggregate bandwidth does not vary much,
because the data streams from main memory.
The next two small sizes probably fit in cache, so they show a higher
aggregate bandwidth than from main memory.
The smallest dataset size is anomalous: for all but the single-core
case it is probably the thread synchronization that overwhelms the actual
performance.
  ...
  \begin{multicols}{2}
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot table[y=1]{stride-frontera100.csv};
        \addplot table[y=11]{stride-frontera100.csv};
        \addplot table[y=22]{stride-frontera100.csv};
        \addplot table[y=33]{stride-frontera100.csv};
        \addplot table[y=44]{stride-frontera100.csv};
        \addplot table[y=56]{stride-frontera100.csv};
      \end{axis}
    \end{tikzpicture}
    \columnbreak
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot table[y=1]{stride-frontera1000.csv};
        \addplot table[y=11]{stride-frontera1000.csv};
        \addplot table[y=22]{stride-frontera1000.csv};
        \addplot table[y=33]{stride-frontera1000.csv};
        \addplot table[y=44]{stride-frontera1000.csv};
        \addplot table[y=56]{stride-frontera1000.csv};
      \end{axis}
    \end{tikzpicture}
    \end{multicols}
...
Only at large strides does the bandwdith demand increase
sufficiently to become a factor.

However, for a larger data set data is streamed from main memory
which does not have enough bandwidth for all cores.
We then see the access time per word go up with the stride size.
...
Many processors will happily compute with
  denormals,
...
\emph{flush-to-zero}\index{floating point numbers!subnormal!flush to zero} where
any subnormal number is set to zero;
this is specified by a compiler flag~\n{-ftz}.
Then we show the slowdown associated with correct handling
of denormals, specified by a compiler flag~\n{-no-ftz}.
Table~\ref{fig:perf-underflow} reports both modes
in an \indextermbus{Intel}{Cascade Lake}, an \indextermbus{AMD}{Milan},
and an \indextermbus{Apple}{M2} processor.
We see that `true' handling of denormals can have severe performance penalties.

\begin{figure}
  \begin{tabular}{rp{1.2in}p{1.2in}p{1.2in}}
                 &Intel Cascade Lake&AMD Milan&MBP M2\\
    Flush-to-zero&1.5               &1.4      &1.2\\
    No Ftz       &45                &1.7      &47\\
  \end{tabular}
  \caption{Access time per word for subormal computing in different processors}
...
for (i=0; i<N; i++)
  s += a[i]*b[i];
\end{lstlisting}
The fact that \lstinline{s} gets both read and written halts the addition
...
Above you learned that data from L1 can be processed with lower latency
and higher bandwidth than from~L2, and L2 is again faster than L3 or memory.
Therefore, if a code has the potential to reuse data
you want to write it in such a way that reused data fits in cache
at some level, the higher --~meaning closer to the process~-- the better.
...
\begin{lstlisting}
blockstart = 0;
for (int b=0; b<datasize/l1size; b++) {
  for (int i=0; i<NRUNS; i++) {
    for (int j=0; j<l1size; j++)
      array_data[blockstart+j] = 2.3*array_data[blockstart+j]+1.2; 
  }
  blockstart += l1size;
}
\end{lstlisting}
...
\begin{remark}
  ...
  blocking is not a transformation that compilers are always allowed to make.
...
While blocking is an essential technique
in operations such as the matrix-matrix product kernel,
the above example is too simple.
In this section we explore blocking in more detail.
...
This runs into a couple of unforeseen problems.
...
for (int irepeat=0; irepeat<how_many_repeats; irepeat++) {
  for (int iword=0; iword<cachesize_in_words; iword++)
    data_array[iword] += 1.1;
}    
\end{lstlisting}
may seem to depend linearly on the array size,
without any influence from being cache contained.
...
(For the \indextermbus{Intel}{compiler} use \lstinline[language=shell]{-qopt-report}.)
...
In an attempt to find an example that does not show this loop exchange
and observe cache influence
you can try to make the inner loop too complicated
for the compiler to analyze,
as would be the case in applications for irregular access patterns.
You could for instance turn the array into a sort of \indexterm{linked list}
that you traverse:
\begin{lstlisting}
// setup
for (int iword=0; iword<cachesize_in_words; iword++)
    data_array[iword] = (iword+1) % cachesize_in_words
...
\begin{lstlisting}
for (int iword=0; iword<cachesize_in_words; iword++)
    data_array[iword] = random() % cachesize_in_words
\end{lstlisting}
  ...
  \tikzsetnextfilename{cachesize-bandwidth}
    \begin{tikzpicture}
      \begin{semilogxaxis}[
        ...
        \addplot table[y=frontera]{cachesize-frontera.csv};
        \addplot table[y=ls6]{cachesize-frontera.csv};
        \addplot table[y=linear]{cachesize-frontera.csv};
      \end{semilogxaxis}
    \end{tikzpicture}
    %% \columnbreak
    %%  \end{multicols}
...
We illustrate this in figure~\ref{fig:core-bandwidth}.
For the \indextermbus{TACC}{Frontera} machine
with \indextermbus{Intel}{Cascade Lake} processors
we plot the linear traversal (top line) and the random traversal.
For the \indextermbus{TACC}{Lonestar6} machine
with \indextermbus{AMD}{Milan} processors
we plot random traversal.
The dataset sizes corresponding to L1 and L2 caches
are clearly visible.
Exceeding the L2 cache has a drop-off in performance,
but not as drastic as for the L1 cache.

Let's recap what we have learned here.
First of all, cachesize will \emph{a~priori} have an influence on code performance:
the more cache-contained, the better.
But secondly: if the code mostly traverses data linearly, the prefetcher
makes the cache effects felt far less;
only code that jumps somewhat randomly through its data
is sensitive to being cache-contained.
...
In all cases we see that the access time per element
goes up dramatically when cache associativity is exceeded.
...
\begin{exercise}
  Analyze this example. When is \lstinline{x} brought into cache, when is it
...
for i=1..n
  for j=1..n
    for k=1..n
      c[i,j] += a[i,k]*b[k,j]
\end{lstlisting}
can only be optimized to keep \lstinline{c[i,j]} in register:
\begin{lstlisting}
for i=1..n
  for j=1..n
    s = 0
    ...
    c[i,j] += s
\end{lstlisting}
Using loop tiling we can keep parts of~\lstinline{a[i,:]} in cache,
assuming that \lstinline{a} is stored by rows:
\begin{lstlisting}
for kk=1..n/bs
  for i=1..n
    for j=1..n
...
people advocate \emph{cache oblivious programming}~\cite{Frigo:oblivious}. 

%%packtsnippet oblivious

Cache oblivious programming can be described as a way of programming
that automatically uses all levels of the
\indextermbus{cache}{hierarchy} without knowing the specific sizes.
This is typically done by using a
\indexterm{divide-and-conquer} strategy, that is, recursive
subdivision of a problem.

A~simple example of cache oblivious programming is the \indextermbus{matrix}
...
improves on the simple one by a factor approximately equal to the cacheline size.
...
explicitly designed to make optimal use of caches;
see section~\ref{sec:goto-gemm}.
%%~\cite{GotoGeijn:2008:Anatomy}.
...
% LocalWords:  Frontera strided columnwise rowwise FFT SOR
