...
The matrix-matrix multiplication algorithm has three loops,
each of which we can block (section~\ref{sec:coding-cachesize}),
...
that underlies the Goto implementation;
for a detailed discussion see~\cite{GotoGeijn:2008:Anatomy}.
...
We start by writing the product $C\leftarrow A\cdot B$ (or
  $C\leftarrow C+AB$ according to the \ac{BLAS} standard) as a sequence of
  low-rank updates:
\[ C_{**} += \sum_k  A_{*k}B_{k*} \]
...
\item Those elements of~$C$ are accumulated, so they stay in register
  ...
  and~$B$; there are no stores until the elements are fully formed!
...
\item Since the same block of~$A$ is used for many successive slivers
  ...
  to let it stay in L2 cache. Now we only stream~$B$.
...
This algorithm is available in optimized libraries
such as \intelmkl, BLIS~\cite{BLIS1}, OpenBLas~\cite{OpenBLAS:github}.
It can also be implemented in specialized hardware, as in the
