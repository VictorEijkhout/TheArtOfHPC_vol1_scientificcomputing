...
such as boolean \indexterm{satisfiability} problems.
...
\begin{exercise}
  Prove that $E=1$ implies that all processors are active all the time.

  (Hint: suppose all processors finish their work in time~$T$,
  except for one processor in $T'<T$. What is $T_p$ in this case?
  Explore the above relations.)
\end{exercise}
...
\indexterm{overhead} as the difference \[ T_o = pT_p-T_1. \]
...
of the sequential fraction of the code: $T_P\rightarrow
...
Thus, we start out with an observed $T_p$, and reconstruct $T_1$ as:
\[ T_1=F_sT_p+p\cdot F_pT_p. \]
...
\begin{exercise}
  Suppose you have an algorithm that can be executed with $E_p\equiv1$.
  What does that mean for the critical path analysis?
\end{exercise}
...
Posted:  5:34pm EST, Mon Nov 25/85,
...
co-processor, the \indextermbus{Intel}{8087 co-processor}, added to it.
This co-processor was very
...
% LocalWords:  parametrized satisfiability runtimes superlinear iso
