...
is.
The most popular is the TOP500 list, maintained at
...
%
\indexterm{Linpack} is a
package for linear algebra operations, no longer in common use, since it
...
kernel discussed in section~\ref{sec:gemm}.
(Subsequently using the factorization to solve a linear system
involves recursion and is therefore harder to optimize;
however, it is of a lower order of complexity, so of lesser importance,
and consequently not discussed in this book.)
It also has the property
that the amount of work outweighs the amount of
memory traffic and network communication:
...
solution through iterative 
  methods (section~\ref{sec:iterative}),
for instance, is much less efficient in a flops-per-second sense, being
