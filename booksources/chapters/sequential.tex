...
\end{enumerate}
This picture,
known as  the \indextermbus{von Neumann}{architecture},
...
memory is too slow to load data into the processor at the rate the
processor can absorb it.
Specifically, a single load can have a latency of
hundreds of cycles,
meaning that this much time elapses between a processor requesting a memory
item and it actually arriving for processing.
On the other hand, a processor can perform several operations per cycle.
  After this long wait for a load, the next load can come faster,
  but you may still be limited by the \indextermdef{bandwidth}
  of the memory,
  which is lower than what the processor can operate on.

To circumvent this imbalance,
a~modern \ac{CPU} has several memory levels in between the
...
contributes to this.
Apart from the frequency, the bandwidth of a bus is
...
\Level 1 {Streaming operations}

%%packtsnippet streaming
The worst case, performancewise speaking, of an operation
is a so-called `streaming' kernel.
Here, data streams from and to memory, with effectively no use of caches
...
\acp{CPU} can do in one clock cycle by using pipelines.
However, each
iteration needs two numbers loaded and one written
for a total of 24 bytes of memory traffic.
(Actually, the cacheline of \lstinline{a[i]} is loaded before it can be written,
so there are 4 memory accesses with a total of 32 bytes per
iteration.)
Typical memory bandwidth figures (see
for instance figure~\ref{fig:cache-specs}) are nowhere near 24 (or~32) bytes per
cycle.
This means that, without caches, algorithm performance can be
...
a = b + c
\end{lstlisting}
can be thought of as being implemented as 
\begin{itemize}
\item load the value of \lstinline{b} from memory into a register,
\item load the value of \lstinline{c} from memory into another register,
\item compute the sum and write that into
  the same or yet another register, and
\item write the sum value back to the memory location of~\lstinline{a}.
\end{itemize}
Looking at \indextermbus{assembly}{code}
(for instance the output of a compiler; section~\ref{sec:assem-out})
you see explicit load, compute, and store instructions,
such as
...
%%packtsnippet registerresident

Registers are a first way of minimizing expensive data movement.
For instance, a strict interpretation of these instructions:
\begin{lstlisting}
a = b + c
d = a + e
\end{lstlisting}
would see \lstinline{a} written to main memory after the first instruction,
and the reloaded for the second.
As an obvious optimization, the computed value of \lstinline{a} could be left in register:
we say that \lstinline{a} stays \indextermsub{resident in}{register}.
This is typically performed as a
\indextermbus{compiler}{optimization}.
...
\end{lstlisting}
However, 
compilers can often figure out such transformations by themselves,
if you set the \indextermbus{optimization}{level} high enough;
  see \CARPref{sec:compile-single}.
...
for i=1,length
  a[i] = b[i] * c
\end{lstlisting}
the quantity \lstinline{c} will probably be kept in register by the compiler,
but in 
\begin{lstlisting}
for k=1,nvectors
  for i=1,length
...
hold~\lstinline{c[k]}. In~C, you can give a hint to the compiler
...
%%packtsnippet cachewhy

To motivate the use of caches let's suppose a variable \lstinline{x} is used twice,
and that it does not stay resident in a register:
\begin{lstlisting}
...
\begin{itemize}
\item load \lstinline{x} from memory into register; operate on it;
...
\begin{itemize}
\item load \lstinline{x} from memory into cache, and from cache into register;
  operate on it;
\item do the intervening instructions;
\item request \lstinline{x} from memory, but since it is still in the cache,
  load it from the cache into register; operate on it.
\end{itemize}
Loading from cache is faster
than loading from main memory in two ways.
The load has a shorter \indextermdef{latency},
meaning the delay between the data being needed in an instruction,
and it actually being available.
Caches also have a higher \indextermdef{bandwidth},
meaning the number of bytes per second transferred.
Thus, if a computation can use data from cache
it will be faster than if it relies on data from main memory.
Caches are fairly small, so arranging a computation in this manner
may take some sophisticated programming.
...
Thus
cache use and reuse is outside of direct programmer control.
Later, especially in sections \ref{sec:locality}
and~\ref{ch:performance}, you will see how it is possible to
influence cache use indirectly.

\Level 2 {Cache hits and misses}

%%packtsnippet cachehitmiss

Data needed in some operation gets copied into the various
caches on its way to the processor.
If, some instructions later, a data item is needed again,
it is first searched for in the L1 cache;
if it is not found there, it is searched for in the L2 cache;
if it is not found there,
it is loaded from further cache levels or main memory.
Finding data in cache is called a
\indextermbus{cache}{hit}, and not finding it a \indextermbus{cache}{miss}.

The most common type of cache miss is the `capacity miss', where more data
is accessed than fits the cache.
The process of cache data being overwritten by data loaded later
is referred to as the earlier data being
\emph{flushed}\index{flush, cache|see{cache, data flushing}}\index{cache!data flushing}.
This is illustrated in figure~\ref{fig:capacity}.
...
Caches can be \emph{private}\index{cache!private} to a core,
as is typically the case for the L1 cache,
or \emph{shared}\index{cache!shared} between some or all of the cores.

The L1 cache is small, typically around 16~or~32 Kbyte.
Level~2 (and, when present, level~3) cache is more plentiful,
up to several
megabytes, but it is also slower, with longer latency and lower bandwidth.
Figure~\ref{fig:cache-specs} has some representative numbers.
Unlike main memory, which is
expandable, caches are fixed in size.
If a version of a processor 
exists with a larger cache, it is usually considerably more expensive.

\begin{figure}[ht]
  %% \begin{quote}
  %% \includegraphics[scale=.6]{spr_specs}
  %% \end{quote}
...
% Draw the triangle sections from bottom to top
\fill[blue!20] (bottomleft) -- (bottomright) -- (top) -- cycle;

% Draw horizontal lines to create sections
\draw[thick] ($(bottomleft)!0.25!(top)$) -- ($(bottomright)!0.25!(top)$);
\draw[thick] ($(bottomleft)!0.5!(top)$) -- ($(bottomright)!0.5!(top)$);
\draw[thick] ($(bottomleft)!0.75!(top)$) -- ($(bottomright)!0.75!(top)$);

% Draw the outer triangle
\draw[thick] (bottomleft) -- (top) -- (bottomright) -- cycle;

% Inside labels
\node[align=center] at (0,7) {L1:\\ 48 KiB/core};
\node[align=center] at (0,5) {L2: 2MiB/core};
\node[align=center] at (0,3) {L3: 2Mb/slice,\\roughly 1 slice/core};
\node[align=center] at (0,1) {DDR5 /HBM memory};

% Left labels (latency)
\node[align=right, text width=3cm, anchor=east] at (-4.2,7) {Latency in cycles:\\ 5};
\node[align=left, anchor=east] at (-4.2,5) {16};
\node[align=left, anchor=east] at (-4.2,3) {80};
\node[align=left, anchor=east] at (-4.2,1) {200 / 260};

% Right labels (bandwidth)
\node[align=left, text width=3cm, anchor=west] at (4.2,7) {Bandwidth (Gb/sec/core):\\ 384};
\node[align=right, anchor=west] at (4.2,5) {128};
\node[align=right, anchor=west] at (4.2,3) {51};
\node[align=right, anchor=west] at (4.2,1) {6 / 30};

\end{tikzpicture}

  \caption{Memory hierarchy of an Intel Sapphire Rapids processor}
  \label{fig:cache-specs}
\end{figure}

%%packtsnippet end

Figure \ref{fig:cache-specs} illustrates the basic facts of the
\indextermbus{cache}{hierarchy}, in
this case for the \indextermbus{Intel}{Sapphire Rapid} chip: the
...
\item Loading data from registers is so fast that it does not
  ...
  hand, there are few registers.
  %% Each core has 16 general purpose
...
%%packtsnippet replacement

Data in cache is placed there by the system, outside of
programmer control.
Likewise, the system decides when to overwrite
data in the cache if it is not referenced in a while,
...
\emph{flushed}\index{cache!data flushing|textbf}
(sometimes: \emph{evicted}\index{cache!data evicted|see{cache, data flushed}})
from cache.
meaning that it is overwritten with the new data,
and is therefore no longer accessible. LRU is by far the most common
...
moved is called a
\indextermdef{cacheline}, sometimes called a
\indextermbus{cache}{block}.  A~typical cacheline is 64 or 128 bytes
long, which in the context of scientific computing implies 8 or 16
double precision floating point numbers. The cacheline size for data
moved into L2 cache can be larger than for data moved into L1 cache.

A first motivation for cachelines is a practical one
of simplification:
if a cacheline is 64~bytes long, six fewer bits
...
\message{CHECK VISUAL PAGE BREAK}
...
since any memory access costs the transfer of several words
%% (see section~\ref{sec:coding-cacheline} for some examples).
An efficient program then tries to use the other items on the cacheline,
...
for (int i=0; i<N; ++i)
  ... = ... x[i] ...
\end{lstlisting}
In figure~\ref{fig:stride-1} we have illustrated this case with 4 words per cacheline.
Requesting
...
\end{wrapfigure}
%
A larger stride, illustrated in figure~\ref{fig:stride-3}
\begin{lstlisting}
for (int i=0; i<N; i+=stride)
  ... = ... x[i] ...
\end{lstlisting}
implies that in every cacheline only certain elements
are used. We illustrate that with stride~3: requesting the first
element loads a cacheline, and this cacheline also contains the
...
same holds for the fourth element. Loading four elements now needs
loading three cachelines instead of one, meaning that two-thirds of
the available bandwidth has been wasted. (This second case would also incur
three times the latency of the first,
if it weren't for the hardware \indexterm{prefetch}
...
are 8K from each other for every~\lstinline{i}, so the last
...
\item The data at \texttt{a[0][0]} is brought into cache and
  ...
  this element, a whole cacheline is transferred.
\item The data at \texttt{a[1][0]} is brought into cache (and
  register, as we will not remark anymore from now on), together
  with its whole cacheline, at cost of some latency. Since this cache
  line is mapped to the same location as the first, the first cache
  line is overwritten.
\item In order to write the output, the cacheline containing
  \texttt{a[2][0]} is brought into memory. This is again mapped to the
  same location, causing flushing of the cacheline just loaded
  for~\texttt{a[1][0]}.
\item In the next iteration, \texttt{a[0][1]} is needed, which is
  on the same cacheline as \texttt{a[0][0]}. However, this cacheline
  has been flushed, so it needs to be brought in anew from main memory
  or a deeper cache level. In doing so, it overwrites the cacheline
  that holds \texttt{a[2][0]}.
\item A similar story holds for \texttt{a[1][1]}: it is on the cache
  line of \texttt{a[1][0]}, which unfortunately has been overwritten in
  the previous step.
\end{itemize}
If a cacheline holds four words, we see that each four iterations of
...
flush data to make room for any incoming item.
This is called a \indextermsub{fully associative}{cache},
and while this design seems optimal, it is also
very costly to build, and slower in use than a direct mapped cache.
...
as in the above example. In that example, a value of $k=3$
...
operand. In practice, subsequent data requests are often adjacent or
...
\begin{figure}[ht]
  \includegraphics[scale=.075]{prefetch}
...
from two consecutive cachelines, and automatically issue a request
for the next following cacheline. This process can be repeated or
extended if the code makes an actual request for that third cache
line. Since these cachelines are now brought from memory well before
they are needed, prefetch has the possibility of eliminating the
latency for all but the first couple of data items.

Prefetch streams typically can not cross
\indextermsub{small}{page} boundaries, so it pays to
...
Some codes benefit from larger page sizes, for instance of a megabyte.
This can be set at the \ac{OS} level.
...
cachelines in the L1 cache of the \indextermbus{AMD}{Barcelona} chip
are 16 words long, divided into two interleaved banks of 8 words. This
means that sequential access to the elements of a cacheline is
...
addresses that the program uses to the actual addresses in memory.
This translation has to be dynamic: if a program is run twice,
the same data can be mapped to two different locations.
A~program has a `logical data
space' (typically starting from address zero) of the addresses used in
the compiled code, and this is translated during program
...
\indextermbus{TLB}{miss}, and the page lookup table is then searched.
This is another case of \indextermsub{spatial}{locality}:
accessing (many) elements that are spread far apart
incurs a performance penalty from \ac{TLB} misses.
%%packtsnippet end
...
\item It is not possible to extract more \acf{ILP}
  ...
  branch prediction makes it 
  impossible (see section~\ref{sec:pipelinecpu}).
\end{itemize}
...
multiple processing \indextermp{core}.
The separate cores can work on unrelated tasks, or,
by splitting a large amount of data parallelism
into parallel tasks
collaborate on a common task at a higher
overall performance~\cite{Olukotun:1996:single-chip}.
\begin{remark}
...
were in effect two processors on the same die, later generations
incorporated L3 or L2 caches that were shared between the two
processor cores; see figure~\ref{fig:core-caches}.
\begin{figure}[ht]
  %%\includegraphics[scale=.14]{cache-hierarchy}
  \includegraphics[scale=.5]{single_dual_cache}
...
\begin{figure}[ht]
  \includegraphics[scale=.75]{amd_milan}
  \caption{Structure of the AMD multicore processors}
  \label{fig:amd-rome}
\end{figure}
...
having its own registers.
Currently, \acp{CPU} with dozens of cores are common
in supercomputers.
Such chips can have complicated cache structures
where an L2 cache is neither private, nor globally shared,
but instead shared by an 8-core `chiplet';
see figure~\ref{fig:amd-rome} for an example.
...
than one processing element has a copy of the same data item. The problem of
ensuring that all cached data are an accurate copy of main memory is
referred to as \emph{cache coherence}:
if the copy in one cache is altered, 
any other copy needs to be updated too, or at least marked as invalid.

In distributed memory architectures, a dataset is usually partitioned
disjointly over the processors,
so conflicting copies of data can only
...
However, cores typically have some private
...
needs access to the item again, it needs to reload that cacheline.
  ...
  cacheline to the other cores.
  This strategy probably has a higher overhead,
...
%% (This makes updating memory locations an \indexterm{atomic
%%   operation}; more about this in section~\ref{sec:shared-lock}.)
However, it will slow down the computation, and it uses bandwidth
that could otherwise be used for loading or storing operands.

The state of a cacheline with respect to a data item in main memory
is usually described in terms of a protocol such as 
the \acf{MSI} coherence protocol, where a
cacheline can be in the following states on a given core:
\begin{description}
\item [Modified:] the cacheline has been modified, and needs to be
  ...
  is evicted, or it is done immediately, depending on the
  write-back policy.
...
from another cache, which may be faster.
(Finding whether a line exists
in state M or~S on another cache,
is called \indexterm{snooping}; an alternative
  is to maintain cache directories; see below.)
If the line is Shared, it can now simply be copied; if it is in state~M in
...
\begin{exercise}
  ...
  indicate which actions cause memory bandwidth to be used.
    (This list
...
\begin{lstlisting}[language=cpp]
  double x,y;
\end{lstlisting}
will likely allocate \lstinline{x} and~\lstinline{y} next to each other in memory, so
there is a high chance they fall on the same cacheline. Now if one
core updates~\lstinline{x} and the other~\lstinline{y}, this cacheline will
...
if the threads all updated the \lstinline{global_result} variable),
this code may have low performance, since the cacheline(s) with the
\lstinline{local_result} array will continuously be invalidated.

That said, false sharing is less of a problem in modern CPUs
than it used to be.
%%packtsnippet end
Let's consider a code that explores the above scheme:
%
...
to it. This derives from the fact that a cluster \indextermdef{node} can
...
%%packtsnippet datareuse
\label{sec:reuse}
\label{sec:intensity}

In the previous
  sections
you learned that processor design is unbalanced
in the sense that loading data is slower than executing the actual operations.
...
\begin{quote}
  ...
  Roughly equivalently, we define the \indextermbus{reuse}{factor}
  as the number of times each loaded data item is used.
\end{quote}
...
loop. The arithmetic intensity here is~$1$.
...
which is important, for instance in \ac{DL} applications.
...
\begin{exercise}
  The matrix-matrix product, considered as mathematical operation, clearly
  has data reuse by the above definition. Argue that this reuse is not
  trivially attained by a straightforward implementation as a triple loop.
  What determines whether this implementation has reuse of data that is in cache?
\end{exercise}
...
\Level 2 {Fourier transform}

Another algorithm that has potential for data reuse
is the \acf{FFT}.
Here, $O(N\log N)$ operations are done on $O(N)$ data,
so there is an arithmetic intensity of~$\log N$.
We will not further go into this.
...
which is a statement about how the ideal algorithm, not its implementation,
...
%
\begin{figure}[t]
  \includegraphics[scale=.08]{roofline1}
  \includegraphics[scale=.08]{roofline2}
  \includegraphics[scale=.08]{roofline3}
...
\item The number of operations per second is also limited by the
  ...
  \]
  If the first factor is~1 we have a streaming operation,
  and performance is limited by bandwidth.
...
factors lower the ceiling.
For instance, if an algorithm fails to use
...
Since using data in cache is faster than getting data from main
memory, a programmer obviously wants to maximize cache reuse
in order to get higher performance.
While placing data in cache is not under explicit
programmer control, even from assembly language,
in most \acp{CPU} it is still
possible to know what data is in cache, and to some extent to control it.

%%packtsnippet locality

In the context of performance optimization,
...
\index{spatial locality|see{locality, spatial}}.
Temporal locality is the easiest to explain: this
describes the use of a data element within a short time of its last use.
If the second reference comes quickly enough after the first,
data will still be in cache and is therefore quicker accessible.

Spatial locality is harder to explain, and we will do so
in detail later.
The basic idea is that after one data item has been referenced,
others that are in some sense `close to it' will be quicker to load.

%%packtsnippet end

\Level 2 {Temporal locality}

%%packtsnippet temporallocality

\begin{figure}[t]
  \includegraphics[scale=.5]{capacity_miss}
  \caption{Large vector flushes old data from cache}
  \label{fig:capacity}
\end{figure}

As an example of \indextermsub{temporal}{locality}, consider the repeated traversal
of a long vector:
\begin{lstlisting}
...
next use; see figure~\ref{fig:capacity}.
Therefore, the use of \lstinline{x[i]} does not exhibit temporal locality:
subsequent uses are spaced too far apart in time for an array element
to remain in cache. 
...
therefore more likely to remain in the cache.
This rearranged code displays better
temporal locality in its use of~\lstinline{x[i]}.
(On the other hand, the original code is more likely
to use vector instructions. The loss of these counteracts
the performance gain from the cache to an extent.)

%%packtsnippet end

\Level 2 {Spatial locality}

%%packtsnippet spatiallocality
...
with caches, the story is different. 

Since data is moved in \indextermbus{cache}{line}s rather than
...
\end{lstlisting}
  ...
  \indexterm{stride}~\lstinline{s}.

  Let \lstinline{S}~be the cacheline size, then as \lstinline{s} ranges from $1\ldots\mathtt{S}$,
  the number of elements used of each cacheline goes
  down from~\lstinline{S} to~1. Relatively speaking, this increases the cost
...
\begin{exercise}
Consider the following code, and assume that \lstinline{nvectors} is small
compared to the cache size, and \lstinline{length} large.
\begin{lstlisting}
...
\begin{itemize}
\item[\lstinline{c[i,j]}] In the first implementation, \lstinline{c[i,j]} is
  ...
  In the second implementation, \lstinline{c[i,j]} will be loaded and stored
  ...
  implementation.
\item[\lstinline{a[i,k]}] In both implementations, \lstinline{a[i,k]} elements are
  ...
  \lstinline{a[i,k]}~is invariant in the inner loop, which constitutes
  ...
  the first case.
\item[\lstinline{b[k,j]}] The two implementations differ greatly in how they
  access the matrix~$B$. First of all, \lstinline{b[k,j]}~is never invariant
  so it will not be kept in register, and $B$~engenders $n^3$~memory
  loads in both cases. However, the access patterns differ.

  In second case, \lstinline{b[k,j]}~is accessed by rows,
  so there is good spatial locality: cachelines will be fully utilized
  after they are loaded. 

  In the first implementation, \lstinline{b[k,j]}~is accessed by
...
for (int i=0; i<N; ++i)
  a[i] = b[i]+1;
\end{lstlisting}
touches every element of \lstinline{a} and~\lstinline{b} once, so there will be a cache miss
for each element.
\end{exercise}
